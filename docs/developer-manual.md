...
2. **「测试连接」是可选的自检**。想当场确认账号填对没,弹窗里点「保存并测试连接」一步到位,
   ...
   它验的是「这个账号能不能登进库」(跑一句 `SELECT 1`,不碰任何业务表)。若账号登得进去、
   但进不去数据源上配的那个默认库,会显示**连接成功 + 一句黄字提示**(平台会绕开那个库继续连接)——那种账号照样能取数,
   前提是任务 SQL 写全限定表名(`库名.表名`)。**库 / 表权限的真正答案在试跑**,
...
| Hive 连接失败：…Permission denied: user […] does not have [USE] privilege on [库名] | 团队账号进不去那个库。平台会自动绕开它继续连接,所以**只有写裸表名的 SQL 才会真的挂** | 任务 SQL 里把表名写全（`库名.表名`），或让数仓管理员给这个账号授该库的权限 |
