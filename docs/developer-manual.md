...
1. **先转作者**。在任务列表里逐个从 ⋮ 菜单进「转移作者」，交给同团队的在职成员。
...
| 作者 | — | 建任务时就是你，不在表单里。转岗或离职时，在任务列表的 ⋮ 菜单里「转移作者」交给**同团队**的在职成员；转移后你不再能编辑它（除非你是团队管理员），详见 [1.1](#11-团队) |
...
| `task_author_transfer` | 转移任务作者（记 from/to、所属团队、以什么身份发起，以及被清掉的冗余编辑权） |
