...
> 对应 2026-08-05 的线上功能。界面与手册不一致时**以系统实际显示为准**，并把差异反馈给平台管理员。
...
1. 打开团队页的「团队取数账号」,列表里是全部数据源,逐个点「配置」,填本团队在该库上的用户名和密码
   (Hive 在 `NONE` 认证方式下可不填密码);
2. 点 **「测试连接」**。只有测通的账号才算就绪;
3. 状态列会显示 `未配置` / `已配置 · 未测通` / `已测通`,并显示「最后由谁、什么时候改的」。

必须知道的四条规则:

- **未就绪 = 任务不能上线、也不能运行**。上线时会直接被拦下;已上线的任务如果团队账号后来失效了,
  业务同学点运行会收到一条指名**团队**的报错,同时该团队的**全体团队管理员**会收到站内 / 飞书通知。
- **改过用户名或密码后,测通状态会被清空**,需要重新点一次「测试连接」。这是有意的——
  否则一套改错了的密码会顶着旧的测通记录继续放行。⚠️ 但请注意:团队账号是共享的,
  **一次改动会让该数据源上本团队的全部任务一起变为未就绪、集体停摆**,直到重新测通。
  前端保存时会为此专门确认一次。
...
| 团队《XX》尚未配置数据源《YY》的取数账号 / 尚未通过连接测试 | 该团队的取数账号未就绪 | 你是该团队的团队管理员就去「我的团队 → 团队取数账号」配置并点「测试连接」（见 [1.2](#12-团队取数账号数据权限)）；否则把这条报错转给团队管理员 |
| 任务上线被拦下：…取数账号… | 上线卡点 | 同上；注意**编辑已上线任务也算一次上线**，所以保存时同样会检查 |
| 连接失败：Access denied for user '…' | 填的团队库账号或密码不对 | 在数据库客户端里用同一套账号验证，再回来重填并重新测试连接 |
...
- [ ] **所属团队在该数据源上的取数账号已「已测通」**（编辑器里数据源下方会提示；未就绪则上线会被拦下）
