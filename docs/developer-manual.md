...
| 运行被授权的任务、预览下载结果 | ✅ | ✅ | ✅ |
| 看**全部**任务（含别人的草稿、已下线） | ✖ | ✅ | ✅ |
| 新建 / 编辑 / 上线 / 下线**任意**任务 | 仅自己建的 | ✅ | ✅ |
| 测试运行（试跑）、SQL 预览、测试枚举 SQL | ✖ | ✅ | ✅ |
| 给人授权（查看 / 运行 / 下载） | 仅自己建的 | ✅ | ✅ |
| 看某任务下**所有人**的运行记录 | 只看自己的 | ✅ | ✅ |
| 改用户角色 / 配置数据源 / 查审计日志 | ✖ | **✖** | ✅ |

**开发者之间是平等的**：你能看见并编辑其他开发者建的任务，别人也能编辑你的。任务列表标题旁的
**只看我的** 勾选框就是为此准备的——勾上后卡片与上方的「已上线 / 草稿 / 总数」计数同时收窄到你自己作者的任务。

**这三件事必须找管理员**：新增或修改数据源、给同事改平台角色、检索审计日志。左侧菜单里的
「用户管理 / 数据源 / 审计」对你不可见。
...
1. **点右上「新建任务」**，填任务名称、选数据源（数据源决定 SQL 方言与默认超时）。
...
| 数据源 | 是 | 下拉里是管理员配好的，格式「名称 （引擎）」。**它决定 SQL 方言和默认超时**，改数据源等于换方言 |
...
被授权「运行」的人会出现在卡片右下的头像组里。**你、其他开发者、管理员对任务天然有全部权限**，
不出现在这份名单里，也不算进头像组。
...
| 只有作者或管理者可操作该任务 / 需要开发者或管理员权限 | 角色不足 | 确认自己的角色标签是「开发者」；若显示「普通用户」找管理员改角色 |
...
| `permission_grant` / `permission_revoke` | 授予任务权限 / 撤销任务权限 |
| `submit_query` / `run_query` / `run_query_failed` / `download` | 提交取数 / 运行取数 / 取数失败 / 下载结果 |
...
| 授权 | `Permission`，动作 `view / run / download` |
