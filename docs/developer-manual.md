...
> **别和「运行编号」混**：那是每跑一次给一个的流水号，出现在运行记录与开放 API 的 `id` 里。排障时说清楚是哪一个 —— 两者会同屏出现。

| 字段 | 必填 | 说明 |
|---|:--:|---|
| 任务名称 | 是 | 业务方在卡片上看到的名字；也是结果文件名的前缀（`<任务名>_<运行时间>.csv`，如 `日报_20260925202030.csv`） |
...
4. **确认下载** 才真正下载完整 CSV（UTF-8 BOM，Excel 直接打开不乱码），文件名是 `<任务名>_<运行时间>.csv`（如 `日报_20260925202030.csv`，即 2026-09-25 20:20:30 开始跑的那次；复用来的结果带的是原始那次运行的时间）；**重新运行** 则回到表单换参数。
...
> **开放 API 触发的运行不推通知**（飞书和站内都没有，成功失败都一样）。调用方在轮询 job 状态，结果与失败原因都在轮询响应里；而 API 是高频的，不静音会把这个人自己在网页上跑的那几条通知一起淹掉。运行记录照常有，带 `source=api`。**唯一的例外是「缺团队取数账号」那条告警**——它发给团队管理员而不是调用方，API 触发照发（同一任务对同一个人 60 分钟内只发一次，免得高频调用刷屏）。
...
| 列 | 说明 |
|---|---|
| 类型 | **正式** = 业务填参取数（会发通知）；**试跑** = 你在编辑器点「测试运行」（不发通知）；**API** = 开放 API 触发（不发通知，调用方在轮询）；**定时** = 订阅计划自动跑（通知推给订阅者，不推发起人）；**补推** = 有编辑权的人把某次正式取数的结果手动推给订阅者（运行人一栏写明是谁补推的） |
| 状态 | 排队 → 运行中 → 成功 / 失败 |
...
| 结果 | 成功且未过期：**预览**（前 50 行）/ **导出**；超过 7 天显示 **已过期**；失败显示 `-`。有编辑权的人在成功的正式取数行上还会看到 **推送给订阅者**（不能推时置灰，悬停看原因） |

业务反馈「取数报错」时的顺序：运行记录 → 找到那条失败记录 → 看**参数**和**执行SQL** → 复制 SQL 自己跑一遍。

**定时运行失败后补推。** 平台不会自动重跑失败的那一期。修好之后：点任务卡片 **运行** 正常取数一次 →
在运行记录里 **预览** 确认数据对 → 点那一行的 **推送给订阅者**。这次的结果（同一个文件，不重跑 SQL）
会作为本期订阅数据推给全部订阅者，订阅者收到「本期数据已补发」。限制：

- 只能推**本期计划时刻之后**、**当前上线版本**跑成功的正式取数（或 API 触发）结果；试跑、旧版本、上一期的结果都不行，
  免得把过时的数当成本期推出去。每条记录只能推一次。
- 本期已经推过一版（准时成功过、或补推过）时，再推就是**替换本期**：订阅者收到「本期数据已更新，请以此版为准」，
  旧版进入常规保留期；「连续未查看」只按期算，看过本期任意一版都算看过，不会因为更正多记一期。
- 本期定时运行还在排队 / 运行时不能推，等它结束。
- 有这个任务编辑权的人都能推（平台管理员、团队管理员、作者、被授予编辑权的人），每次都记审计「补推结果给订阅者」。
...
1. **团队收窄分两套口径**。**任务类**（绝大多数指标）看任务现在归谁（`sql_templates.team_id`），
   ...
   **人类**（目前只有 API Token 那两项）看团队成员——token 属于人，不属于任务，强行套任务归属只会算错。
   两套并存是有意为之。
2. **时间窗是半开区间** `[开始, 结束)`。相邻两个区间因此首尾相接、不重不漏，环比才算得对。
...
| 指标 | 算法 | 吃时间范围 |
|---|---|---|
| 正式取数 / 定时运行 / API 调用 | 按 `query_jobs.source` 分开计数，各一张卡；趋势图四条线（含作者试跑）。**永不合并成一个「总运行次数」**。`has_data` 看**全期**——「这段时间没人调」与「从来没人调过」指向不同的下一步 | 是 |
| 活跃取数人 | 发起过 `source=run` 的去重人数，带与上一个等长区间的环比。**不含试跑**——否则开发者调试很勤会被读成业务很活跃 | 是 |
| 下载转化 | 下载次数 ÷ 成功运行次数。偏低说明跑出来的结果没被真正用上 | 是 |
| 业务自助率 | 普通用户（`role=user`）发起的正式取数 ÷ 全部正式取数 | 是 |
| 近 7 天用过的 Token | `api_token_last_used_at` 落在最近 7 天内的 token 数 / 此刻 `api_token_hash` 非空的人数（每人至多一枚，吊销即三列一起清空、不再计入）。**固定 7 天**（`API_TOKEN_ACTIVE_DAYS`），刻意不吃时间范围——它问的是「这些长期凭证还活着吗」。写入按 60 秒节流（`LAST_USED_WRITE_INTERVAL_SECONDS`），精度到分钟。团队视角按**团队成员**收窄（见约定 1）。一枚 token 没发过、也从没有过 API 调用时，这两张卡整行不摆 | **否** |

> **刻意不做「节省了多少人天」**：它需要一个「人工取一次数要多久」的假设，那是拍脑袋；
> 更糟的是一旦写进看板就会变成 KPI，反过来污染数据（刷运行次数）。
...
| 执行耗时 P90 | 只取成功且有耗时的运行，**按引擎分开**。Hive 与 MySQL 的超时上限差一个量级，混算的分位数没有可比性。用最近秩不用插值——分位数必须是样本里真实存在过的值 | 是 |
| 等待超过 1 分钟 | `started_at - created_at` ≥ 60 秒的次数。**排除试跑**（同步执行、不入队）。没有 `started_at` 的历史运行不参与，也**不当成零等待**；一条样本都没有时是「—」 | 是 |
| 成功但 0 行 | 跑通了却没有数据，多半是参数填错或那天确实没数 | 是 |
| 此刻队列 | 当前 `queued` / `running` 的条数。**只在团队视角显示**——全平台视角由下面的实时负载板接管，同一个数不摆两处 | **否** |

#### 实时负载（仅全平台视角）

回答「平台此刻扛不扛得住」。接口 `GET /api/analytics/live`，只对全平台视角开放：团队管理员访问返回 403，平台管理员切到团队视角时这一块也不显示。
这样限定的原因是，worker 槽位全平台共用。团队视角下如果显示「满载」却只列本队一条在跑，这个画面自相矛盾，看的人没法据此行动。如果把全部在跑的都列出来，又等于越权展示。

这一块不受时间范围影响。页面在前台时每 15 秒刷新一次，切到后台就暂停。

| 指标 | 算法 |
|---|---|
| 槽位占用 | 正在执行的非试跑运行数 / `WORKER_CONCURRENCY`。超过超时上限 60 秒后仍是 `running` 的行视为已中断：它们要等孤儿回收才会转成 `failed`，所以不算占槽，单独注明条数。槽位上限读的是 API 进程的配置，前提是它和 worker 读同一份 `config.ini` |
| 排队 | `queued` 的条数，以及最早一条已等了多久。明细按 `query_service.claim_order` 排序，也就是 worker 真实的认领顺序（非 API 来源优先），最多列 20 条 |
| Worker | **按队列推断，不是心跳**。按下面的顺序判断，命中第一条即停：①开着 `RUN_INLINE` 时显示「内联执行」；②没有排队时显示「空闲」；③有排队且未中断的在跑数 ≥ 槽位时显示「满载」；④有排队、槽位没满，但最早一条已等了超过 `max(60 秒, 5 × WORKER_POLL_INTERVAL)` 时显示「疑似停摆」；⑤其余显示「正常」 |
| 编辑器试跑 | `source=test` 且 `running` 的条数。试跑在 API 进程里同步执行，不占 worker 槽位，所以单独计数 |
| 今日分时 | 按库时钟，从今天 0 点到当前小时。每小时给三个数：提交次数（按 `created_at`）、峰值并发、排队等待 P90。峰值并发把每次执行看作半开区间 `[started_at, 结束)` 扫描得出：结束时间取 `started_at + duration_ms`，失败的运行没有耗时，用 `updated_at`；还在跑的算到当下为止。排除试跑、补推和复用，这三种都没进过队列 |
| 未来 24 小时定时 | 按整点分桶，范围是当前这个小时加上之后 24 小时。只统计计划开着、任务已上线、且有订阅者的计划，这和调度器实际建 job 的条件一致。计划时刻由 `subscription_service.planned_between` 计算，它的命中规则和 `latest_planned_at` 一致，并有交叉校验测试。某小时的运行数超过槽位的 2 倍就标为扎堆 |

#### 运行明细

回答「具体是哪几次、卡在哪、报了什么」。实时负载只看此刻，运行健康只给聚合；运维排障要落到某一次运行，就用这一块。
接口是 `GET /api/analytics/runs`，单条详情是 `GET /api/analytics/runs/{id}`。收窄方式与其余板块相同（`job_conditions`），平台视角和团队视角都开放：团队管理员只看得到本团队任务的运行。详情接口对范围外的 id 一律返回 404，不区分「不存在」和「看不见」。

| 项 | 口径 |
|---|---|
| 落窗 | 按提交时间 `created_at` 落进时间范围 |
| 默认范围 | 包含补推和复用记录，行上带标记。运维查「那人说没收到数」时要看得见这两种记录。勾选「只看真正执行的」后排除这两种，与运行健康同口径 |
| 排队 | `started_at - created_at`。没有 `started_at` 的显示「—」，不当成零 |
| 耗时 | 只算执行时间，不含排队。失败的运行不写 `duration_ms`，用 `updated_at - started_at` 近似，并标「≈」。不补这个近似的话，按耗时排序时超时失败会全部沉底，而它们恰恰是最该先看的 |
| 状态计数 | 状态芯片上的数字**不带状态筛选本身**，这样选了「失败」之后，其余状态照样显示有几条 |
| 按归因筛选 | 与运行健康的失败归因共用同一套规则（`classify_error`）。只在最近 5000 条失败里查找，超出时界面会提示 |
| 错误 | 列表里只给归因和前 200 字，完整原文、执行 SQL 和参数都在详情里 |
| 排序与分页 | 可按提交时间（默认）、耗时最长、排队最久排序。每页最多 100 条 |

运行健康的失败归因和数据源失败数，以及实时负载的在跑/排队，都可以一键带着条件跳到这里（下钻）。
...
| 没有团队管理员 | 一个团队管理员都没有的团队数（仅平台视角）。没人能给它配取数账号、加成员、授编辑权 | 否 |

#### 团队管理员看不到的

平台级的内容整块不下发（**键在响应里都不存在**，不是给个 0）：目前只剩「没有团队管理员的团队」
这一张卡。采纳、运行健康、任务资产三块两种视角的键**完全相同**（测试钉着），新加平台专属键要先想清楚。

**审计派生的指标一条都不给团队管理员**（目前整页也没有审计派生的指标）：审计接口本身是管理员专属，运营分析不该开一条绕过它的侧门；
...
采纳板里开放 API 那两张卡：token 按团队成员收窄，API 调用按任务归属收窄。
两套收窄口径并存是有意为之——token 属于人，运行属于任务，
