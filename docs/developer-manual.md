...
- 用户在界面「**API Token**」入口生成自己的 token（形如 `rk_...`），**每人一个**，可重置 / 吊销；
  重置后旧 token 立即失效。
...
| 指标 | 算法 | 吃时间范围 |
|---|---|---|
| 已发放 Token | 此刻 `users.api_token_hash` 非空的人数，**每人至多一枚**；吊销会把哈希 / 签发时间 / 最近使用三列一起清空，清空即不计入。团队视角按**团队成员**收窄——这是全表唯一一处不按任务归属收窄的指标（token 属于人，不属于任务） | **否** |
...
> 页面上「近 7 天用过」的卡片标题里带着「近 7 天」四个字，不是装饰：它是全页唯一一个
> 既不是「本区间」也不是「此刻」的窗口，标题不写清楚就会有人拿它跟按 30 天算的调用数对账。
