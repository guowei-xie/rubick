...
任务列表标题旁的 **只看我的** 勾选框把范围进一步收窄到你自己作者的任务；属于多个团队时旁边还会
出现一个**团队下拉**。两者都先于上方「已上线 / 草稿 / 总数」计数生效，所以数字和卡片永远一致。
...
2. **账号进不去数据源那个默认库时,填「入口库」**。Hive 建连必须先进入某一个库,而**能进哪个库
   取决于这套账号的授权范围** —— 一个数据源被多团队共用时,各团队账号的范围本就不同。
   在「团队取数账号」的配置弹窗里把「入口库」填成这个账号能进的库(如本部门的库),留空则用
   数据源的默认库。它**不改变 SQL 怎么写**:写全限定表名(`库名.表名`)照样能跨库取数。
3. **「测试连接」是可选的自检**。想当场确认账号填对没,弹窗里点「保存并测试连接」一步到位,
   或在列表里单独点「测试连接」。不测也一样能上线、能取数;
   它验的是「这个账号能不能登进库」,并顺带**列出这个账号能访问的库**(跑一句
   `SHOW DATABASES`,不碰任何业务表)。它**刻意不依赖数据源上配的那个 Database** ——
   那只是不写库名时的解析起点,进不去它并不代表账号不可用。**库 / 表权限的完整答案在试跑**,
   那才与上线后的取数同一套身份、同一条 SQL;
4. 状态列会显示 `未配置` / `已配置 · 未验过` / `已测通`,并显示「最后由谁、什么时候改的」。
...
![任务列表（开发者视角）：状态筛选片、只看我的、回收站与新建任务按钮](assets/dev-tasks.png)
...
| 查询超时（秒） | 否 | 留空按引擎默认：MySQL 120 秒、Hive 3600 秒。**只作用于业务的正式取数**，不影响试跑与枚举查询 |
...
| **测试运行** | 用各变量的**测试值**真跑一次，最多取 50 行，弹出「试跑结果」浮窗（返回行数 · 列数，可查看执行 SQL）。所有变量都要有测试值才跑。<br>**超时固定 120 秒**（不吃任务上的「查询超时」，也不吃 Hive 的 3600 秒）——长耗时 Hive 任务试跑可能超时，属正常，可先用小时间范围的测试值验证写法 |
...
- **结果最多 10 万行，超出部分被直接丢弃且界面没有额外提示**。可能超量的任务：自己在 SQL 里加
  `LIMIT`（想让业务控制就写 `LIMIT :n` 并把变量设为数值型），或在任务说明里要求业务分批取。
...
![普通用户的任务列表：只有被授权的已上线任务，没有「新建任务」「只看我的」「回收站」](assets/dev-user-tasks.png)
...
1. 点右上 **运行并预览**，屏幕上方出现「已提交，执行中…」并一直转。
...
卡片 **⋮ → 运行记录**。你能看到这个任务下**所有人**的运行（普通用户只能看到自己的）。
...
| 状态 | 排队 → 运行中 → 成功 / 失败 |
| 行数 | 成功时的结果行数——**比预期少很多就要怀疑撞了 10 万行上限** |
| 参数 | **查看(N)** 看业务这次填的每个值，复现问题最快的入口 |
| 执行SQL | **查看** 看参数已代入的最终语句，可直接复制到客户端里跑 |
...
| Hive 连接失败：…does not have [USE] privilege on […]，且报错说"平台依次试了 X、Y 都被拒" | 这个账号进不去平台试过的任何库，而 Hive 建连必须先进入一个库，所以它**当前无法取数**（与 SQL 怎么写无关） | 在「团队取数账号」里把「入口库」填成这个账号能进的库；或让数仓给它授权其中任一个库（有 `USE` 权限即可） |
| Hive 连接失败：…Permission denied: user […] does not have [USE] privilege on [库名] | 团队账号进不去那个库。取数时平台会自动绕开它继续连接,所以**只有写裸表名的 SQL 才会真的挂**;「测试连接」压根不碰这个库,故不会因此报错 | 任务 SQL 里把表名写全（`库名.表名`），或让数仓管理员给这个账号授该库的权限。点一次「测试连接」看列出的库,就知道这个账号能取哪些库 |
...
- **超时**：MySQL 默认 120 秒、Hive 默认 3600 秒；任务上可单独设「查询超时（秒）」覆盖（只影响正式取数）。
- **枚举 SQL 固定 120 秒**，请指向小维表。
- **值列表规模**：业务选超过 5000 个值会收到「过多可能导致查询过慢或失败」的提示；真要批量匹配大列表，考虑改成 join 维表的写法。
- **10 万行上限**是静默截断，自己在任务说明里写清「一次最多取多久的数据」。
...
- [ ] **测试运行**跑通，行数与列名符合预期（行数贴近 10 万时加 LIMIT 或提示分批）
...
| 试跑超时 / 取样行数 | 120 秒 / 50 行 | 编辑器「测试运行」，不吃任务超时 |
| 枚举 SQL 超时 | 120 秒 | 「测试」与业务「更新枚举值」 |
| 单次结果行数上限 | 100,000 行 | 超出静默丢弃 |
...
**平台当前没有的能力**（别按这些做方案）：字段级 / 行级脱敏、结果水印、定时自动取数、枚举候选值定时自动刷新
