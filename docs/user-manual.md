...
- **「测试连接」是可选的自检,不是必须走的一步。** 账号存下来就生效,任务即可上线、可运行;
  ...
  那只是自检痕迹失效,**任务照跑**。
  它验的是「账号能不能登进库」。若登得进去、但进不去数据源上配的默认库,你会看到
  **连接成功 + 一句黄字提示**:那种账号照样能取数,但任务 SQL 得把表名写全(`库名.表名`),
  否则会报「找不到表」。要么让作者改 SQL,要么找数仓管理员给账号授那个库的权限。
...
| Database | 否 | 默认库名。它是**首选**而不是硬要求：取数账号进不去它时，平台会自动绕开它继续连接，此时任务 SQL 必须把表名写全（`库名.表名`） |
...
**测试连接** 成功提示「连接成功」，失败会带上数据库返回的原因；账号登得进去但进不去上面那个 Database 时，提示会变成一句黄字说明（仍算连通）。**删除**需二次确认（「删除后无法恢复；仍被任务引用时无法删除。」）——只要还有任务在用这个数据源就删不掉，会提示被几个任务使用。
