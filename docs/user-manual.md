...
> 本手册对应 2026-08-18 的线上功能（含团队、团队取数账号、跨团队隔离）。界面与手册描述不一致时，**以系统实际显示为准**，并把差异反馈给平台管理员。
...
卡片 **⋮ → 运行记录**，抽屉标题「运行记录：<任务名>」。**被授权的业务用户只看到自己跑过的**；
该任务**所属团队的成员**与平台管理员看到这个任务下所有人的运行（含试跑）。
别的团队的开发者看不到这个任务，自然也看不到它的运行记录。
...
| 团队取数账号 | 团队管理员 | 配置本团队在各数据源上的数据库账号与「入口库」（「测试连接」是可选的自检） |
...
- **「测试连接」是可选的自检,不是必须走的一步。** 账号存下来就生效,任务即可上线、可运行;
  ...
  那只是自检痕迹失效,**任务照跑**。
  它验的是「账号能不能登进库」,并会**列出这个账号能访问的库**(如「该账号可访问 3 个库:
  default、finance_bp、…」)—— 配完账号最该确认的就是这一条。它与数据源上配的 Database
  无关,所以不会因为那个库没授权就报连不上。任务 SQL 只要把表名写全(`库名.表名`),
  能访问的库都能取;想取列表里没有的库,得找数仓管理员给账号授权。
- **账号进不去数据源那个默认库时,填「入口库」。** 配置弹窗里的「入口库」= 这套账号建连时进入
  的库,留空则用数据源的默认库。为什么需要它:Hive 建连必须先进入某个库,而能进哪个库取决于
  账号的授权范围——同一个数据源被多个团队共用时,各团队账号的范围本就不同。填了它**不改变 SQL
  怎么写**:写全限定表名(`库名.表名`)照样能跨库取数。改动它会清空「已测通」标记(那条痕迹
  只对旧配置成立),但**任务照跑**。
...
> **Hive 的一条硬约束**：建立连接时必须先进入某一个库（驱动会执行 `USE <库>`）。所以取数账号至少要能进入一个库——平台会依次尝试上面的 Database 与 `default`，两个都没权限时**这个账号无法取数**（与 SQL 怎么写无关，连接都建不起来）。报错会把试过的库列出来；解法是让数仓给账号授权其中任一个库（有 `USE` 权限即可）。
