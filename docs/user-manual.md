...
![取数抽屉：任务说明、单值参数、值列表参数与「获取枚举值」「上传/粘贴列表」入口](assets/manual-run-form.png)

控件下方的灰色小字「示例：…」是作者填的测试值，照它的格式填不会错。

**值列表**下面可能出现这些辅助按钮（取决于作者有没有开启）：

- **获取枚举值**：跑作者预置的一段查询，把候选值拉出来供勾选。旁边会显示「作者测试约 N ms，供参考」，据此判断要等多久。取回后提示「取到 N 个候选值，请勾选」；一次最多返回 1000 个，超出会提示「已截断」。
...
| 枚举值获取 SQL | 仅值列表 | 业务点「获取枚举值」时跑，**返回一列候选值**；旁边 **测试** 按钮可当场验证，成功会显示取到多少个、耗时多少毫秒，并把耗时存下来给业务参考 |
...
**平台当前没有的能力**（别按这些做方案）：字段级/行级脱敏、结果水印、定时自动取数、产品内提需求单、按部门/用户组授权、自定义 RBAC 角色。
...
| 枚举值获取 SQL | 作者预置的一段查询，业务点「获取枚举值」时跑出候选值 |
