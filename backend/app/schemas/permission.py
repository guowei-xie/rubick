...
class SubjectIn(BaseModel):
    """「这次操作针对哪个人」的入参形状。授权与代订阅共用一份 —— 下面那条邮箱禁令是提权防线,
    抄成两份迟早只改到其中一份。"""

    subject_type: Literal["user"] = "user"  # 仅支持对个人操作
    # 对象二选一:已知平台用户传 subject_id;通讯录搜出的新人传 subject_open_id + 展示资料,
    # 由服务端在「真正要用他」这一刻才按 open_id 落库(壳用户不再在搜索时生成,避免用户表膨胀)。
