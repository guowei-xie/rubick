...
def probe(ds: DataSource, credential: Credential) -> None:
    """用某个身份连一次目标库,失败转成可读错误。

    团队凭证测通(verify)与管理员测数据源连通性(routes/datasources)共用这一处,
    否则两边的错误文案会各自漂移 —— 而开发者手册的报错速查表是按这个前缀写的。
    """
    try:
        get_connector(ds, credential).test_connection()
...
def verify(db: Session, cred: TeamDataSourceCredential) -> None:
    ...
    try:
        probe(ds, Credential(cred.username, cred.password, owner_team_id=cred.team_id))
